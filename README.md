...
[//]: # "start-toc"
   ...
   * [Искусственная жизнь, симуляция эволюции](#искусственная-жизнь-симуляция-эволюции)
      ...
      * [Платформы](#платформы)
         * [Машинки](#машинки)
         * [Квадратные клетки](#квадратные-клетки)
         * [Существа с мышцами](#существа-с-мышцами)
...
|                                                        | Визуализация | Метод оптимизации     | Видео | Исходный код | Исполняемый файл | Мозги             | Турбо | Основа                      |
|--------------------------------------------------------|--------------|-----------------------|-------|--------------|------------------|-------------------|-------|-----------------------------|
| Tierra                                                 | -            | Естественный отбор    | +     | +            | +                | Программа         |       | ОЗУ                         |
| Слепой часовщик                                        | 2D           | ?                     | +     | +            | -                | -                 |       | Часы                        |
| Эволюция с мышцами                                     | 2D           |                       |       |              |                  |                   |       | Существа с мышцами          |
| Сражающиеся кружочки                                   | 2D           |                       |       |              |                  |                   |       | Круги с пулями              |
| Машинки на произвольных нейросетях                     | 2D           | ?                     | +     | -            | -                | Нейросеть         | ?     | Машинки                     |
| Learn 2D Stick                                         | 2D           | Генетический алгоритм | +     | -            | +                | Нейросеть         | +     | Существа с мышцами          |
| foo52ru "боты"                                         | 2D           | Естественный отбор    | +     | +            | -                | Программа         |       | Квадратные клетки           |
| foo52ru деревья                                        | 2D           | Естественный отбор    | +     | +            | -                | -                 |       | Квадратные клетки           |
| SnakeAI                                                | 2D           | Генетический алгоритм | +     | +            | -                | Нейросеть         |       | Змейка                      |
| Другие змейки?                                         |              |                       |       |              |                  |                   |       |                             |
| Tallensy evolution                                     | 2D           | Генетический алгоритм | -     | -            | +                | -                 | -     | Машинки⬡                    |
| Амёбы                                                  | 2D           | Генетический алгоритм | -     | -            | +                | ?                 | +     | Амёбы                       |
| gencar.co                                              | 2D           | Генетический алгоритм | -     | +            | web              | -                 | +     | Машинки+                    |
| Cephalopods                                            | 2D           | Генетический алгоритм | -     | +            | web              | Spiking нейросеть | +     | Cephalopods                 |
| WANN                                                   | 2D           | Эволюционный алгоритм | -     | +            | -                | WANN              | +     | Существа с мышцами, машинки |
| Evolved Virtual Creatures                              | 3D           | Генетический алгоритм | +     | -            | -                | Skiping нейросеть | -     | Существа с мышцами          |
| Flexible Muscle-Based Locomotion for Bipedal Creatures | 3D           | CMA-ES                | +     | -            | -                | ?                 | -     | Существа с мышцами++        |
| Онигири                                                | 3D           | Генетический алгоритм | +     | +            | -                | Нейросеть         | ?     | Существа с мышцами          |
| Framsticks                                             | 3D           | ?                     | ?     | ?            | ?                | ?                 | ?     | ?                           |
| Polyworld                                              | 3D           | Естественный отбор    | +     | +            | -                | Нейросеть         | ?     | Существа с мышцами          |
|                                                        |              |                       |       |              |                  |                   |       |                             |

## Платформы

**Платформа** в данном контексте - это основа на которой будет строиться искусственная жизнь, задачи к ней. Мутации.

### Машинки

Существо - машинка, обычно 2D, либо с видом сверху, либо сбоку.

Обычно они либо:
* Обучают свою нейросеть, чтобы с информацией от сенсоров корректно водить
* Меняют своё тело, чтобы на данной трассе пройти как можно дальше

Никаких сложных задач им больше не ставят. Да и особо не поставишь, потому что, например, задача создания автономного автомобиля - это задача другой области алгоритмов, не искусственной жизни.

Зато за счёт такой простоты любой алгоритм оптимизации легко сходится к решению: находит нужную форму тела, либо доводит нейросеть.

Можно использовать для первого раза, потому что очень просто программируется.

Для вида сверху есть формулы для написания собственной машинки: [gamedev.ru](http://gamedev.ru/code/articles/?id=4233&page=6).

### Квадратные клетки

Существо - 2D клетка. 

Преимущество данной платформы в том, что можно реализовать
...
Первая программа благодаря которой я познакомился с тем как работает симуляция эволюции. Имеет подробную справку
...
* [ ] Составить таблицу всех симуляций
* [ ] Вытащить из каждой симуляции на чём она основана и рассмотреть эти основы
